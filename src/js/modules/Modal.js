export class Modal {
  ...
  init() {
    ...
    document.addEventListener(this.currentEvent, (e) => {
    ...
    this.$close.forEach(close => {
      close.addEventListener(this.currentEvent, this.hadleClose.bind(this));
    });

    document.addEventListener("keydown", event => {
      if (event.key === "Escape" && this.$modalId.classList.contains(this.activeOverlayClass)) {
        this.closeModal();
      }
    })
  }
  ...
  closeModal() {
    this.closeAllModals();
    setTimeout(() => {
      this.$modalId.classList.remove(this.activeOverlayClass);
